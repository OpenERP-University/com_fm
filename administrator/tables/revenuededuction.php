...
/**
 * revenuededuction Table class
 */
class FmTablerevenuededuction extends JTable {
    ...
    public function __construct(&$db) {
        parent::__construct('#__fm_revenue_deduction', 'id', $db);
        JTableObserverContenthistory::createObserver($this, array('typeAlias' => 'com_fm.revenuededuction'));
    }
    ...
    public function bind($array, $ignore = '') {
        ...
        if (($task == 'save' || $task == 'apply') && (!JFactory::getUser()->authorise('core.edit.state', 'com_fm.revenuededuction.' . $array['id']) && $array['state'] == 1)) {
            $array['state'] = 0;
        }
        // them vao
        if ($array['id'] == 0) {
        ...
        //Support for multiple or not foreign key field: employee_guid
        if (!empty($array['employee_guid'])) {
            if (is_array($array['employee_guid'])) {
                $array['employee_guid'] = implode(',', $array['employee_guid']);
            } else if (strrpos($array['employee_guid'], ',') != false) {
                $array['employee_guid'] = explode(',', $array['employee_guid']);
            }
        }
        ...
        if (!JFactory::getUser()->authorise('core.admin', 'com_fm.revenuededuction.' . $array['id'])) {
            $actions = JFactory::getACL()->getActions('com_fm', 'revenuededuction');
            $default_actions = JFactory::getACL()->getAssetRules('com_fm.revenuededuction.' . $array['id'])->getData();
            $array_jaccess = array();
    ...
    protected function _getAssetName() {
        ...
        return 'com_fm.revenuededuction.' . (int) $this->$k;
    }
