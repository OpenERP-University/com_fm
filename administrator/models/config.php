 ...
 * Fm model.
 */
class FmModelConfig extends JModelAdmin
{
	/**
	...
	protected $text_prefix = 'COM_FM';
        public $typeAlias = 'com_fm.config';
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		// Get the form.
		$form = $this->loadForm('com_fm.config', 'config', array('control' => 'jform', 'load_data' => $loadData));
	...
	protected function prepareTable($table)
	{
		...
		if (empty($table->id)) {
			...
			if (@$table->ordering === '') {
				$db = JFactory::getDbo();
				$db->setQuery('SELECT MAX(ordering) FROM #__fm_config');
				$max = $db->loadResult();
