 ...
 */
class FmModelRevenuedeductions extends JModelList {
    ...
    public function __construct($config = array()) {
        if (empty($config['filter_fields'])) {
            $config['filter_fields'] = array(
                ...
                'pay_electricity', 'a.pay_electricity',
                'water_charges', 'a.water_charges',
                'rent', 'a.rent',
                'detain_type', 'a.detain_type',
                'detain', 'a.detain',
    ...
    protected function getListQuery() {
        ...
        $query->from('`#__fm_revenue_deduction` AS a');
        ...
        if (!empty($search)) {
            if (stripos($search, 'id:') === 0) {
                ...
                $query->where('( a.employee_guid LIKE '.$search.'  OR  a.pay_electricity LIKE '.$search.'  OR  a.water_charges LIKE '.$search.'  OR  a.rent LIKE '.$search.'  OR  a.detain_type LIKE '.$search.' )');
            }
        }
    ...
    public function getItems() {
		...
		foreach ($items as $oneItem) {
			...
			$oneItem->employee_guid = !empty($textValue) ? implode(', ', $textValue) : $oneItem->employee_guid;
					...
					$oneItem->detain_type = JText::_('COM_FM_REVENUEDEDUCTIONS_DETAIN_TYPE_OPTION_' . strtoupper($oneItem->detain_type));
		}
        return $items;
    }
