...
if ($saveOrder)
{
	$saveOrderingUrl = 'index.php?option=com_fm&task=e_allowances.saveOrderAjax&tmpl=component';
	JHtml::_('sortablelist.sortable', 'e_allowanceList', 'adminForm', strtolower($listDirn), $saveOrderingUrl);
}
...
<?php endif;?>
		...
		<table class="table table-striped" id="e_allowanceList">
			...
			<?php foreach ($this->items as $i => $item) :
                ...
                <?php if (isset($this->items[0]->state)): ?>
					<td class="center">
						<?php echo JHtml::_('jgrid.published', $item->state, $i, 'e_allowances.', $canChange, 'cb'); ?>
					</td>
                <?php endif; ?>
                    
				<td>

				 <a href = "<?php echo JRoute::_('index.php?option=com_fm&view=e_allowance&layout=edit&id=' . $item->id); ?>"  >	<?php echo $item->employee_guid; ?></a>
				</td>
				<td>
