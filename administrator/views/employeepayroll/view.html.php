 ...
 * View to edit
 */
class FmViewEmployeepayroll extends JViewLegacy {
    ...
    protected function addToolbar() {
        ...
        JToolBarHelper::title(JText::_('COM_FM_TITLE_EMPLOYEEPAYROLL'), 'employeepayroll.png');

        // If not checked out, can save the item.
        if (!$checkedOut && ($canDo->get('core.edit') || ($canDo->get('core.create')))) {

            JToolBarHelper::apply('employeepayroll.apply', 'JTOOLBAR_APPLY');
            JToolBarHelper::save('employeepayroll.save', 'JTOOLBAR_SAVE');
        }
        if (!$checkedOut && ($canDo->get('core.create'))) {
            JToolBarHelper::custom('employeepayroll.save2new', 'save-new.png', 'save-new_f2.png', 'JTOOLBAR_SAVE_AND_NEW', false);
        }
        // If an existing item, can save to a copy.
        if (!$isNew && $canDo->get('core.create')) {
            JToolBarHelper::custom('employeepayroll.save2copy', 'save-copy.png', 'save-copy_f2.png', 'JTOOLBAR_SAVE_AS_COPY', false);
        }
        if (empty($this->item->id)) {
            JToolBarHelper::cancel('employeepayroll.cancel', 'JTOOLBAR_CANCEL');
        } else {
            JToolBarHelper::cancel('employeepayroll.cancel', 'JTOOLBAR_CLOSE');
        }
    }
